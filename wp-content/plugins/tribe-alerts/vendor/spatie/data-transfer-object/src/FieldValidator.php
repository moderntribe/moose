...
abstract class FieldValidator
{
    ...
    protected static array $typeMapping = ['int' => 'integer', 'bool' => 'boolean', 'float' => 'double'];
    ...
    private function getCurrentReturnTypesFromDocblock(string $definition) : array
    {
        ...
        \preg_match($DOCBLOCK_REGEX, $definition, $matches);
        $type = $matches[1] ?? null;
        if (!$type) {
            return [];
        }
    ...
    {
        return \array_filter(\array_map(fn(?string $type) => self::$typeMapping[$type] ?? $type, $types));
    }
}
