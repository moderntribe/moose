...
namespace Tribe\Alert_Scoped\Tribe\Libs\Queues\Backends;
...
class WP_Cache implements Backend
{
    public function enqueue(string $queue_name, Message $m)
    {
        ...
        $queue[\uniqid('queue', \true)] = ['priority' => $m->get_priority(), 'task_handler' => $m->get_task_handler(), 'args' => $m->get_args(), 'taken' => \false];
        // Sort by priority
