...
namespace Tribe\Alert_Scoped\Tribe\Libs\P2P;

class P2P_Query extends \Tribe\Alert_Scoped\P2P_Query
{
    ...
    {
        $parent = \Tribe\Alert_Scoped\P2P_Query::create_from_qv($q, $object_type);
        ...
        list($p2p_q, $q) = $parent;
        $local_p2p_q = new \Tribe\Alert_Scoped\P2P_Query();
    ...
    public function alter_clauses(&$clauses, $main_id_column)
    {
        if (!$this->should_override_parent()) {
            return \Tribe\Alert_Scoped\P2P_Query::alter_clauses($clauses, $main_id_column);
        }
        /** @var \wpdb $wpdb */
        global $wpdb;
        $joins = '';
        ...
        foreach ($this->ctypes as $directed) {
            ...
            $to_side = $directed->get('opposite', 'side');
            // opposite == to for the 'any' direction
            $to_join = $wpdb->prepare(" LEFT JOIN {$wpdb->p2p} {$to_alias}\n\t\t\t\t ON {$main_id_column}={$to_alias}.p2p_to\n\t\t\t\t  AND {$to_alias}.p2p_type= %s", $directed->name);
            ...
            $from_side = $directed->get('current', 'side');
            // current == from for the 'any' direction
            $from_join = $wpdb->prepare(" LEFT JOIN {$wpdb->p2p} {$from_alias}\n\t\t\t\t ON {$main_id_column}={$from_alias}.p2p_from\n\t\t\t\t  AND {$from_alias}.p2p_type= %s", $directed->name);
            if (!empty($connected_post_ids)) {
        ...
        foreach ($this->ctypes as $directed) {
            ...
            switch ($directed->get_direction()) {
                ...
                default:
                    ...
                    if (!$to_side instanceof \Tribe\Alert_Scoped\P2P_Side_Post) {
                        return \false;
                    }
                    $from_side = $directed->get('opposite', 'side');
                    if (!$from_side instanceof \Tribe\Alert_Scoped\P2P_Side_Post) {
                        return \false;
                    }
                    break;
            }
