...
use Tribe\Alert_Scoped\Tribe\Project\Post_Types\Page\Page;
/**
 * Class Required_Page
 *
 * @abstract
...
abstract class Required_Page
{
    ...
    {
        $args = ['post_type' => $this->get_post_type(), 'post_status' => 'publish', 'post_title' => $this->get_title(), 'post_name' => $this->get_slug(), 'post_content' => $this->get_content(), 'comment_status' => 'closed', 'ping_status' => 'closed'];
        /**
    ...
    {
        if (empty($this->field_group_key) || !\class_exists('Tribe\\Alert_Scoped\\Tribe\\Libs\\ACF\\Field')) {
            return [];
        }
        $field = new ACF\Field(static::NAME);
        $field->set_attributes(['parent' => $this->field_group_key, 'label' => $this->get_field_label(), 'name' => static::NAME, 'type' => 'post_object', 'post_type' => $this->get_post_type(), 'return_format' => 'int']);
