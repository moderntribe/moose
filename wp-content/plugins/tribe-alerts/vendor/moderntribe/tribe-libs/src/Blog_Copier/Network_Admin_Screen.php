...
class Network_Admin_Screen
{
    ...
    {
        \add_submenu_page('sites.php', \__('Blog Copier', 'tribe'), \__('Blog Copier', 'tribe'), 'manage_sites', self::NAME, [$this, 'render_screen']);
        \add_settings_section(self::SECTION_FORM, \__('Create a Copy', 'tribe'), function () {
            \do_action('tribe/project/copy-blog/form');
        }, self::NAME);
        \add_settings_field('copy-blog-source', \__('Choose Source Blog to Copy', 'tribe'), [$this, 'source_blog_field'], self::NAME, self::SECTION_FORM);
        \add_settings_field('copy-blog-address', \__('New Blog Address', 'tribe'), [$this, 'new_blog_address_field'], self::NAME, self::SECTION_FORM);
        \add_settings_field('copy-blog-title', \__('New Blog Title', 'tribe'), [$this, 'new_blog_title_field'], self::NAME, self::SECTION_FORM);
        \add_settings_field('copy-blog-files', \__('Copy Files?', 'tribe'), [$this, 'copy_files_field'], self::NAME, self::SECTION_FORM);
        \add_settings_field('copy-blog-notification', \__('Notification Recipients', 'tribe'), [$this, 'notification_recipients_field'], self::NAME, self::SECTION_FORM);
    }
    public function render_screen()
    {
        $title = \__('Blog Copier', 'tribe');
        $action = \network_admin_url('edit.php?action=' . self::NAME);
        \ob_start();
    ...
    public function handle_submission()
    {
        ...
        \wp_safe_redirect(\network_admin_url('sites.php?page=' . self::NAME));
        exit;
    }
    /**
