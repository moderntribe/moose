...
class Copy_Manager
{
    ...
    {
        $post_id = \wp_insert_post(['post_type' => self::POST_TYPE, 'post_status' => 'pending', 'post_title' => $configuration->get_title(), 'post_content' => \json_encode($configuration), 'post_author' => $configuration->get_user()]);
        return $post_id;
    }
    ...
    public function schedule_next_step($completed_task, $args)
    {
        ...
        \add_post_meta($args['post_id'], self::TASK_LOG, \json_encode(['timestamp' => \time(), 'task' => $completed_task]));
        ...
        if (\class_exists('Tribe\\Alert_Scoped\\WP_CLI')) {
            \Tribe\Alert_Scoped\WP_CLI::debug(\sprintf('Completed blog copy task: %s', $completed_task));
        }
    }
    /**
    ...
    public function mark_error($failed_task, $args, $error)
    {
        ...
        if (\class_exists('Tribe\\Alert_Scoped\\WP_CLI')) {
            \Tribe\Alert_Scoped\WP_CLI::debug(\sprintf('Error in blog copy task: %s.', $failed_task));
            \Tribe\Alert_Scoped\WP_CLI::debug(\sprintf('Error message: %s.', $error->get_error_message()));
        }
    }
