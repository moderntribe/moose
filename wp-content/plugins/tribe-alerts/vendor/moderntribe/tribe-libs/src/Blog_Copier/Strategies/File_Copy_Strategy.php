...
namespace Tribe\Alert_Scoped\Tribe\Libs\Blog_Copier\Strategies;
...
abstract class File_Copy_Strategy
{
    ...
    protected function get_source_directory($src_blog_id)
    {
        ...
        $from = \str_replace(' ', "\\ ", \trailingslashit($dir_info['basedir']) . '*');
        // * necessary with GNU cp, doesn't hurt anything with BSD cp
    ...
    protected function get_destination_directory($dest_blog_id)
    {
        ...
        $dir_info = \wp_upload_dir();
        $to = \str_replace(' ', "\\ ", \trailingslashit($dir_info['basedir']));
        \restore_current_blog();
        /**
    ...
    protected abstract function copy_files($source, $destination);
}
