...
namespace Tribe\Alert_Scoped\Tribe\Libs\Oembed;

use Tribe\Alert_Scoped\Tribe\Libs\Cache\Cache;
/**
 * Youtube API
 *
...
class YouTube implements \JsonSerializable
{
    ...
    private function request_from_api()
    {
        ...
        $cache_key = [__CLASS__, __METHOD__, 'url' => $this->url];
    ...
    {
        return ['id' => $this->get_id(), 'url' => $this->url, 'title' => $this->get_title(), 'video' => $this->get_html(), 'thumbnail_url' => $this->get_thumbnail_url(), 'description' => $this->get_description(), 'html' => $this->get_html()];
    }
}
