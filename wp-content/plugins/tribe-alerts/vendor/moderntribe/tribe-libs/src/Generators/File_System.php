...
class File_System
{
    public function create_directory($directory)
    {
        ...
        if (!\wp_mkdir_p($directory) && !\is_dir($directory)) {
            \Tribe\Alert_Scoped\WP_CLI::error('Sorry...something went wrong when we tried to create ' . $directory);
        }
    }
    public function write_file($file, $contents, $overwrite = \false)
    {
        if (\file_exists($file) && !$overwrite) {
            \Tribe\Alert_Scoped\WP_CLI::error('Sorry... ' . $file . ' already exists.');
        }
        if (!($handle = \fopen($file, 'w'))) {
            \Tribe\Alert_Scoped\WP_CLI::error('Sorry...something went wrong when we tried to write to ' . $file);
        }
    ...
    public function insert_into_existing_file($file, $new_line, $below_line)
    {
        if (!($handle = \fopen($file, 'r+'))) {
            \Tribe\Alert_Scoped\WP_CLI::error('Sorry.. ' . $file . ' could not be opened.');
        }
        $inserted = \false;
        $contents = '';
        while (!\feof($handle)) {
        ...
        if (!\fclose($handle)) {
            \Tribe\Alert_Scoped\WP_CLI::error('Sorry.. ' . $file . ' an error has occurred.');
        }
        $this->write_file($file, $contents, \true);
    }
    ...
    {
        $object = \json_decode(\str_replace(['(', ')'], ['&#40', '&#41'], \json_encode($array)), \true);
        $export = \str_replace(['array (', ')', '&#40', '&#41'], ['[', ']', '(', ')'], \var_export($object, \true));
        $export = \preg_replace("/ => \n[^\\S\n]*\\[/m", ' => [', $export);
        $export = \preg_replace("/ => \\[\n[^\\S\n]*\\]/m", ' => []', $export);
