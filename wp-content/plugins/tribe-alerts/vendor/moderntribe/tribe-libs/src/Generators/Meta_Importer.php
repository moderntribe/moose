...
class Meta_Importer extends Generator_Command
{
    ...
    {
        return [['type' => 'positional', 'name' => 'field_group', 'optional' => \true], ['type' => 'flag', 'name' => 'delete-group', 'optional' => \true, 'description' => \__('Whether or not to delete the imported field group. Defaults to true, pass --no-delete-group if you wish to preserve the group.', 'tribe'), 'default' => \true]];
    }
    ...
    {
        $locations = [];
        $accepted_locations = ['post_type' => 'post_types', 'taxonomy' => 'taxonomies', 'options_page' => 'settings_pages', 'user_form' => 'users'];
    ...
    {
        $object_class = \trailingslashit(\dirname(__DIR__, 2)) . 'Object_Meta/' . $this->class_name . '.php';
    ...
    protected function class_file_template()
    {
        ...
        return \sprintf($class_file, $this->class_name, $this->slug, $this->field_keys(), $this->title, $this->add_field_functions($this->group['fields']), $this->field_functions($this->group['fields']), $this->field_constants());
    }
    ...
    protected function field_functions($fields)
    {
        ...
        foreach ($fields as $field) {
            ...
            $fields_containing_subfields = ['repeater', 'group', 'flexible_content'];
            if (!\in_array($field['type'], $fields_containing_subfields)) {
                $functions .= \sprintf($function_partial, $this->sanitize_slug([$field['label']]), $this->file_system->constant_from_class($this->sanitize_slug([$field['label']])), $this->file_system->format_array_for_file($field, 16));
            } else {
    ...
    private function prepare_field($field)
    {
        ...
        $zero_fields = ['required', 'conditional_logic'];
    ...
    private function get_repeater($field)
    {
        ...
        $group = \sprintf($group_partial, $this->sanitize_slug([$field['label']]), $this->file_system->constant_from_class($this->sanitize_slug([$field['label']])), $this->file_system->format_array_for_file($write_field, 16), $this->add_field_functions($field['sub_fields'], '$repeater'));
