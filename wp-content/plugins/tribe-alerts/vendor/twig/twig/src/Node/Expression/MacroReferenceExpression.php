...
class MacroReferenceExpression extends AbstractExpression implements SupportDefinedTestInterface, CoercesChildrenToStringInterface
{
    ...
    private function compileDynamic(Compiler $compiler) : void
    {
        ...
        if ($this->definedTest) {
            $compiler->subcompile($this->getNode('template'))->raw('->hasMacro(\'macro_\'.')->subcompile($this->getNode('name'))->raw(', $context)');
            return;
        }
        $compiler->subcompile($this->getNode('template'))->raw(\sprintf('->getTemplateForMacro($%s = \'macro_\'.', $var))->subcompile($this->getNode('name'))->raw(', $context, ')->repr($this->getTemplateLine())->raw(', $this->getSourceContext())')->raw(\sprintf('->{$%s}', $var))->raw('(...')->subcompile($this->getNode('arguments'))->raw(')');
    }
}
