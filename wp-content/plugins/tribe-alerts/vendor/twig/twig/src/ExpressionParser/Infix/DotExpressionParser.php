...
use Tribe\Alert_Scoped\Twig\Lexer;
...
final class DotExpressionParser extends AbstractExpressionParser implements InfixExpressionParserInterface, ExpressionParserDescriptionInterface
{
    ...
    public function parse(Parser $parser, AbstractExpression $expr, Token $token) : AbstractExpression
    {
        ...
        $isMacroTarget = $expr instanceof NameExpression && (null !== $parser->getImportedSymbol('template', $expr->getAttribute('name')) || '_self' === $expr->getAttribute('name'));
        if ($isMacroTarget && $attribute instanceof ConstantExpression && \is_string($name = $attribute->getAttribute('value')) && \preg_match('#^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$#D', $name)) {
