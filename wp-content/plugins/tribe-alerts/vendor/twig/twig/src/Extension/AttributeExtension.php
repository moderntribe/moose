...
use Tribe\Alert_Scoped\Twig\Attribute\AsTwigFilter;
use Tribe\Alert_Scoped\Twig\Attribute\AsTwigFunction;
use Tribe\Alert_Scoped\Twig\Attribute\AsTwigTest;
    ...
    private function initFromAttributes() : void
    {
        ...
        foreach ($reflectionClass->getMethods() as $method) {
            ...
            foreach ($method->getAttributes(AsTwigFunction::class) as $reflectionAttribute) {
                ...
                $attribute = $reflectionAttribute->newInstance();
                $callable = new TwigFunction($attribute->name, [$reflectionClass->name, $method->getName()], ['needs_context' => $attribute->needsContext ?? \false, 'needs_environment' => $attribute->needsEnvironment ?? $this->needsEnvironment($method), 'needs_charset' => $attribute->needsCharset ?? \false, 'needs_is_sandboxed' => $attribute->needsIsSandboxed ?? \false, 'is_variadic' => $method->isVariadic(), 'is_safe' => $attribute->isSafe, 'is_safe_callback' => $attribute->isSafeCallback, 'always_allowed_in_sandbox' => $attribute->alwaysAllowedInSandbox ?? \false, 'deprecation_info' => $attribute->deprecationInfo]);
                if ($callable->getMinimalNumberOfRequiredArguments() > $method->getNumberOfParameters()) {
