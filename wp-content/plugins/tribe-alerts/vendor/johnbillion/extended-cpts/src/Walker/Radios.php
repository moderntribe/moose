...
namespace Tribe\Alert_Scoped\ExtCPTs\Walker;
...
class Radios extends \Walker
{
    ...
    public function start_el(&$output, $object, $depth = 0, $args = [], $current_object_id = 0)
    {
        ...
        $output .= "\n<li id='{$args['taxonomy']}-{$object->term_id}'>" . '<label class="selectit">' . '<input value="' . \esc_attr($value) . '" type="radio" name="tax_input[' . \esc_attr($args['taxonomy']) . '][]" ' . 'id="in-' . \esc_attr($args['taxonomy']) . '-' . \esc_attr((string) $object->term_id) . '"' . \checked(\in_array($object->term_id, (array) $args['selected_cats']), \true, \false) . \disabled(empty($args['disabled']), \false, \false) . ' /> ' . \esc_html(\apply_filters('the_category', $object->name)) . '</label>';
    }
    /**
