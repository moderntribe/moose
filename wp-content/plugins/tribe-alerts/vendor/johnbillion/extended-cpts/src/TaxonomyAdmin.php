...
class TaxonomyAdmin
{
    ...
    public function meta_boxes(string $object_type, $object) : void
    {
        ...
        if (\in_array($this->taxo->taxonomy, $taxos, \true)) {
            ...
            \wp_add_inline_script('wp-edit-post', \sprintf('wp.data.dispatch( "%s" ).removeEditorPanel( "taxonomy-panel-%s" );', $store, $this->taxo->taxonomy));
    ...
    protected function do_meta_box(WP_Post $post, ?\Walker $walker = null, bool $show_none = \false, string $type = 'checklist') : void
    {
        ...
        switch ($type) {
            case 'dropdown':
                ...
                $dropdown_args = ['option_none_value' => \is_taxonomy_hierarchical($taxonomy) ? '-1' : '', 'show_option_none' => $none, 'hide_empty' => \false, 'hierarchical' => \true, 'show_count' => \false, 'orderby' => 'name', 'selected' => \reset($selected) ?: 0, 'id' => "{$taxonomy}dropdown", 'name' => \is_taxonomy_hierarchical($taxonomy) ? "tax_input[{$taxonomy}][]" : "tax_input[{$taxonomy}]", 'taxonomy' => $taxonomy, 'required' => $this->args['required']];
    ...
    public function glance_items(array $items) : array
    {
        ...
        $count = \wp_count_terms(['taxonomy' => $this->taxo->taxonomy]);
