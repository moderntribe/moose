 ...
 */
namespace Tribe\Alert_Scoped\Whoops\Exception;
...
class Formatter
{
    ...
    public static function formatExceptionAsDataArray(InspectorInterface $inspector, $shouldAddTrace, array $frameFilters = [])
    {
        ...
        $response = ['type' => \get_class($exception), 'message' => $exception->getMessage(), 'code' => $exception->getCode(), 'file' => $exception->getFile(), 'line' => $exception->getLine()];
        if ($shouldAddTrace) {
